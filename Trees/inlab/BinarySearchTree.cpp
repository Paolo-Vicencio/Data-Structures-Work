...
string BinarySearchTree::pathTo(const string& x)  {
    string theFinal = helperMethod(root, x);
    return theFinal;
}
...
string BinarySearchTree :: helperMethod(BinaryNode*& n, const string& x)  {
    string compString = "";
    if (find(root,x)) {
        if(x == n ->value){
            cout << n-> value;
        ...
        cout << n-> value;
        helperMethod(n->left, x);
    }
        else if (x > n->value) {
        cout << n-> value;
        helperMethod(n->right, x);
        }
    }
...
int BinarySearchTree :: numNodesHelper(BinaryNode* n) const {
    int counter = 1;
    if (n == NULL) {
        return 0;
    }
