...
mergeSort:
	...
	; mov rcx, rdx 		; moves end variable into the rcx area for later
	; mov r8, rsi 		; moves the rsi register (start) into the r8 register for later

start: 
	
	cmp rsi, rdx 		; compares the left most index to the right most index
	jge done 	 		; if they are equal, jump to the end
	; sets up the first recrusion variables
	mov rcx, rsi 		; adds rsi into rdx
	add rcx, rdx
	shr rcx, 1 	 		; using shift to divide by 2
	push rsi
	push rdx
	push rcx
	mov rdx, rcx
	call mergeSort 	; goes through the first recursive iteration 
	pop rcx
	pop rdx
	pop rsi


	; sets up the second recursion variables
	push rcx
	inc  rcx
	push rsi 
	push rdx	
	mov rsi, rcx		  	; moves the previously stored rcx (end point) back into the rdx register
	call mergeSort 	; goes through the second recursive iteration
	pop rdx
	pop rsi
	pop rcx
